...
def insert_weather_table(df):
    """weather_after 테이블 갱신"""

    with engine.begin() as conn:
        conn.execute(text("TRUNCATE TABLE weather_after;"))
    ...
    print("weather_after 테이블 갱신 완료")


def insert_monthly_table(df):
    """weather_after_monthly 테이블에 매 시간 데이터 추가 저장"""

    df.to_sql(
        name="weather_after_monthly",
        con=engine,
        if_exists="append",
        index=False
    )
    print("weather_after_monthly 테이블 누적 저장 완료")
